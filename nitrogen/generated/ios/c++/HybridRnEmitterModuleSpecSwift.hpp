///
/// HybridRnEmitterModuleSpecSwift.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © 2025 Marc Rousavy @ Margelo
///

#pragma once

#include "HybridRnEmitterModuleSpec.hpp"

// Forward declaration of `HybridRnEmitterModuleSpec_cxx` to properly resolve imports.
namespace RnEmitterModule { class HybridRnEmitterModuleSpec_cxx; }

// Forward declaration of `AnyMap` to properly resolve imports.
namespace NitroModules { class AnyMap; }

#include <string>
#include <optional>
#include <NitroModules/AnyMap.hpp>
#include <functional>

#include "RnEmitterModule-Swift-Cxx-Umbrella.hpp"

namespace margelo::nitro::rnemittermodule {

  /**
   * The C++ part of HybridRnEmitterModuleSpec_cxx.swift.
   *
   * HybridRnEmitterModuleSpecSwift (C++) accesses HybridRnEmitterModuleSpec_cxx (Swift), and might
   * contain some additional bridging code for C++ <> Swift interop.
   *
   * Since this obviously introduces an overhead, I hope at some point in
   * the future, HybridRnEmitterModuleSpec_cxx can directly inherit from the C++ class HybridRnEmitterModuleSpec
   * to simplify the whole structure and memory management.
   */
  class HybridRnEmitterModuleSpecSwift: public virtual HybridRnEmitterModuleSpec {
  public:
    // Constructor from a Swift instance
    explicit HybridRnEmitterModuleSpecSwift(const RnEmitterModule::HybridRnEmitterModuleSpec_cxx& swiftPart):
      HybridObject(HybridRnEmitterModuleSpec::TAG),
      _swiftPart(swiftPart) { }

  public:
    // Get the Swift part
    inline RnEmitterModule::HybridRnEmitterModuleSpec_cxx& getSwiftPart() noexcept {
      return _swiftPart;
    }

  public:
    // Get memory pressure
    inline size_t getExternalMemorySize() noexcept override {
      return _swiftPart.getMemorySize();
    }

  public:
    // Properties
    

  public:
    // Methods
    inline double sum(double num1, double num2) override {
      auto __result = _swiftPart.sum(std::forward<decltype(num1)>(num1), std::forward<decltype(num2)>(num2));
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
      auto __value = std::move(__result.value());
      return __value;
    }
    inline void sendNativeEvent(const std::string& message, const std::optional<std::shared_ptr<AnyMap>>& data) override {
      auto __result = _swiftPart.sendNativeEvent(message, data);
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
    }
    inline double addRNFromNativeListener(const std::function<void(const std::string& /* message */, const std::optional<std::shared_ptr<AnyMap>>& /* data */)>& callback) override {
      auto __result = _swiftPart.addRNFromNativeListener(callback);
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
      auto __value = std::move(__result.value());
      return __value;
    }
    inline void removeListener(double id) override {
      auto __result = _swiftPart.removeListener(std::forward<decltype(id)>(id));
      if (__result.hasError()) [[unlikely]] {
        std::rethrow_exception(__result.error());
      }
    }

  private:
    RnEmitterModule::HybridRnEmitterModuleSpec_cxx _swiftPart;
  };

} // namespace margelo::nitro::rnemittermodule
